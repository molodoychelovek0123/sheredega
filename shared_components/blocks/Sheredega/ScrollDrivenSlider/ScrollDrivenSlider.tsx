...
const innerScript = (ref: RefObject<HTMLUListElement>, gap, view) => {
  const slider = ref.current;
  let isDown = false;
  let startX;
  let scrollLeft;

  const itemWidth = slider.querySelector(".scrolldriven-slide").clientWidth + gap;
            ...
            return (
              ...
              <li key={image?._tina_metadata?.name ?? index} name={`${id}__${index}`}
                ...
                <div className={`scrolldriven-container-${aspectRatio} relative w-full`}>
                  <img src={image.src} alt={image.alt} draggable={false} className={`scrolldriven-image`}
                       onClick={typeof window !== "undefined" ? () => window.open(image.link, "_blank") : undefined} />
                </div>
              </li>
            );
          })}
            ...
            onClick={(e) => {
              ...
              if (ref.current) {
                ref.current.scrollTo({
                  left: index * ref.current.querySelector(".scrolldriven-slide").clientWidth,
                  behavior: "smooth"
                });
              }
            }} />
