...
export const StickyAboutPerson = ({
                                    firstBlock,
                                    secondBlock,
                                    indent,
                                    uniquePath,
  ...
  const {
    personName: personNameFirst = "Юрий Шередега",
    personPosition: personPositionFirst = "Руководитель бюро"
  } = firstBlock ?? {};
  const {
    personName: personNameSecond = "Юрий Шередега",
    personPosition: personPositionSecond = "Руководитель бюро"
  } = secondBlock ?? {};
  ...
  const scrollTrigger = useCallback(
    (event) => {
      if (event.target.documentElement.scrollTop + screenHeight / 2 <= (startPosition + endPosition) / 2) {
        setActiveBlock(1);
      } else {
        setActiveBlock(2);
      }
  ...
  `;
  return (
    <div className="h-[300vh] mt-[350px] "
         style={{
           marginTop: -(height / 2),
      ...
      <div className={"sticky top-1/2 transform -translate-y-1/2"} style={{ minHeight: height }}>
        <Container indent={indent} uniquePath={uniquePath} customCss={{ data: myCustomCss }} hide={true}
                   className="relative">
          <div
            ...
            <div
              ...
              <span
                className={cn("text-black  text-base  font-normal block ",
                  ...
                  "text-wrap-balance",
                  " rte-text ")}>
                «<b>Наша миссия — развитие</b> и преображение городских территорий.<br /><br />Поиск центральной площадки для общегородских мероприятий»</span>
                  </div>
